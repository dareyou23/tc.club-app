    ...
    <ProtectedRoute>
      <div>
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold theme-text">Verfügbarkeits-Matrix</h2>
        </div>
        <div className="mb-4">
          <p className="text-xs theme-text-muted mb-2 font-medium uppercase tracking-wide">Mannschafts-Filter</p>
          <div className="flex gap-2">
            {[null, 1, 2, 3, 4].map(m => (
              <button key={m ?? 'alle'} type="button" onClick={() => setFilterM(m)}
                className={`px-5 py-2 rounded text-sm font-medium ${filterM === m ? 'btn-primary shadow' : 'theme-btn-inactive theme-text-muted hover:bg-gray-200'}`}>
        ...
        <div className="space-y-2 mb-6">
          {filteredSt.map((st: any) => {
            ...
            const cardClass = zuWenig ? 'bg-accent-red/10 border-red-200' : 'bg-green-50 border-green-200';
            ...
            return (
              <div key={st.id} className={`rounded-lg border p-3 ${cardClass}`}>
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span className={`text-sm font-bold ${zuWenig ? 'text-accent-red' : 'text-green-600'}`}>{zusagen.length}/6</span>
                </div>
                <div className="text-xs theme-text-muted space-y-0.5">
                  ...
                  {absagen.length > 0 && <p>❌ <span className="text-accent-red">{absagen.map((s: any) => `${s.vorname} ${s.name}`).join(', ')}</span></p>}
                </div>
              </div>
            );
          })}
        ...
        {/* Matrix-Tabelle */}
        <h3 className="font-bold theme-text-muted mb-2">Gesamtübersicht</h3>
        <div className="overflow-x-auto">
          <table className="text-xs border-collapse w-full">
            <thead>
              <tr className="theme-btn-inactive">
                <th className="border border-dove-300 px-2 py-1 text-left sticky left-0 theme-btn-inactive z-10 min-w-[140px]">Spieler</th>
                <th className="border border-dove-300 px-1 py-1 text-left w-12">LK</th>
                <th className="border border-dove-300 px-1 py-1 text-center w-8">K</th>
                {filteredSt.map((st: any) => (
                  <th key={st.id} className="border border-dove-300 px-1 py-1 text-center min-w-[50px]">
                    <div>{formatKurz(st.datum)}</div>
                    <div className="text-[10px] theme-text-subtle">{st.heimspiel ? 'H' : 'A'}</div>
                  </th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filteredSpieler.map((s: any) => {
                ...
                return (
                  <tr key={s.id} className={isKern ? '' : 'theme-text-subtle'}>
                    <td className="border border-dove-300 px-2 py-1 sticky left-0 card z-10 whitespace-nowrap">
                      {s.vorname} {s.name}
                      {festspielMap[s.id]?.festgespielt && (
                        <span className="ml-1 text-accent-red text-[10px]">🔒M{festspielMap[s.id].festgespieltIn.join(',')}</span>
                      )}
                    </td>
                    <td className="border border-dove-300 px-1 py-1 text-center">{s.lk ? Number(s.lk).toFixed(1) : '-'}</td>
                    <td className="border border-dove-300 px-1 py-1 text-center">{s.kern ? `M${s.kern}` : ''}</td>
                    {filteredSt.map((st: any) => {
                      const status = getStatus(st.id, s.id);
                      return (
                        <td key={st.id} className={`border border-dove-300 px-1 py-1 text-center ${STATUS_BG[status]}`}>
                          {STATUS_ICON[status]}
                        </td>
                      );
                    })}
            ...
            <tfoot>
              <tr className="theme-btn-inactive font-bold">
                <td className="border border-dove-300 px-2 py-1 sticky left-0 theme-btn-inactive z-10" colSpan={3}>Zusagen</td>
                {filteredSt.map((st: any) => {
                  const count = filteredSpieler.filter((s: any) => getStatus(st.id, s.id) === 'ja').length;
                  return (
                    <td key={st.id} className={`border border-dove-300 px-1 py-1 text-center ${count < 6 ? 'text-accent-red bg-accent-red/10' : 'text-green-600 bg-green-50'}`}>
                      {count}
                    </td>
                  );
                })}
