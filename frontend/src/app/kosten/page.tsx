  ...
  return (
    <ProtectedRoute>
      ...
      {loading ? (
        ...
        <p className="theme-text-muted">Noch keine Kosten angefallen.</p>
      ) : (
        <div className="space-y-4">
          {konten.map((k, i) => (
            <div key={i} className="card rounded-lg shadow p-4">
              <div className="grid grid-cols-3 gap-4 text-center">
                <div>
                  <p className="text-sm theme-text-muted">Hallengebühren</p>
                  <p className="text-xl font-bold">{k.hallengebuehren?.toFixed(2)}€</p>
                </div>
                <div>
                  <p className="text-sm theme-text-muted">Trainerkosten</p>
                  <p className="text-xl font-bold">{k.trainerkosten?.toFixed(2)}€</p>
                </div>
                <div>
                  <p className="text-sm theme-text-muted">Gesamt</p>
                  <p className="text-xl font-bold theme-link">{k.gesamtkosten?.toFixed(2)}€</p>
                </div>
              </div>
            </div>
          ))}
